...
class Model:
    ...
    def forward(self):
        ...
        if not isinstance(self.config.lender_change, lc.Boltzmann):
            ...
            self.statistics.compute_prob_bankruptcy_max_e_and_c()
    ...
    def do_loans(self):
        logs_extra_relationships_change = self.config.lender_change.extra_relationships_change(self)
        self.log.debug("loans", logs_extra_relationships_change)
            ...
            if demand > 0:
                ...
                if lender is None or lender.d > 0:
                    ...
                    bank.do_fire_sales(rationing_of_bank,
                                       f"rationing={rationing_of_bank:.4f} as no lender for this bank"
                                       if lender is None else
                                       f"rationing={rationing_of_bank:.4f} as lender {lender.get_id(short=True)} "
                    ...
                    num_of_rationed += 1
                    bank.do_fire_sales(rationing_of_bank,
                                       f"lender #{lender.id}.s={lender.s} but need d={demand:.4f}, "
                                       f"rationing={rationing_of_bank:.4f}",
                                       'loans')
    ...
    def replace_bankrupted_banks(self):
        ...
        for possible_removed_bank in self.banks:
            if possible_removed_bank.failed:
                total_removed += 1
                if possible_removed_bank.rationing == 0 and possible_removed_bank.paid_loan > 0:
        ...
        self.log.debug('repay', 'this step ΔD={:.4f} and '.format(
            self.statistics.var_deposits[self.t]) + 'failures={}'.format(total_removed))
    ...
    def do_interest_rate_common_part(self):
        if len(self.banks) <= 1:
            return None
        self.max_e = max(self.banks, key=lambda k: k.E).E
        for bank in self.banks:
            if bank.d > 0: # bank.get_lender() is not None and bank.get_lender().l > 0:
                bank.lambda_ = bank.L / bank.E
                bank.prob_surviving = 0
            else:
                bank.lambda_ = 0
                bank.prob_surviving = self.config.p_avg_ir if self.config.p_avg_ir else bank.E / self.max_e
...
class Bank:
    ...
    def __init__(self, new_id=None, bank_model=None):
        ...
        self.d = 0
        self.B = 0
        self.psi = (self.E / self.model.max_e) if self.model.config.psi_endogenous else None
        self.incrD = 0
        self.paid_profits = 0
        self.paid_loan = 0
        self.incrR = 0
...
class ModelOptimized(Model):
    ...
    # TODO: hacer que la optimizacion funcione igual
