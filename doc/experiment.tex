...
\begin{itemize}
    \item \texttt{kurtosis\_bankruptcies}: Fisher kurtosis of the bankruptcy series
    \item \texttt{hill\_bankruptcies}: Hill tail exponent of the upper tail
    \item \texttt{dist\_to\_default}: mean capital ratio $E/(D+E)$
\end{itemize}
\end{frame}

\begin{frame}{Metric 1: Kurtosis of Bankruptcies}
\centering
\Large
\[
\text{Kurtosis} = \frac{1}{n} \sum_{t=1}^T
\left( \frac{B_t - \bar{B}}{\sigma_B} \right)^4 - 3
\]

\vspace{0.5em}
\normalsize
\noindent
Where $B_t = \sum_i \mathbbm{1}_{\text{failed}}(i)$ at each step $t$.
...
\vspace{0.5em}
\noindent\textbf{Hypothesis:} High $p$ $\Rightarrow$ higher kurtosis.
\end{frame}

\begin{frame}{Metric 2: Hill Exponent}
\centering
\Large
\[
H_k = \frac{1}{k} \sum_{i=1}^{k} \log\left( \frac{B_{(i)}}{B_{(k+1)}} \right)
\]

\vspace{0.5em}
\normalsize
\noindent
$B_{(1)} \geq B_{(2)} \geq \cdots \geq B_{(k+1)}$ are the sorted
bankruptcy counts (upper tail).
...
\vspace{0.5em}
\noindent\textbf{Hypothesis:} High $p$ $\Rightarrow$ lower Hill exponent.
\end{frame}
...
\centering
\Large
\textbf{Three metrics, one hypothesis}

\vspace{1.5em}
\normalsize
\begin{enumerate}
    \item \textbf{Kurtosis} $\uparrow$ $\Rightarrow$ extreme events more likely
    \item \textbf{Hill} $\downarrow$ $\Rightarrow$ heavier tails
    \item \textbf{Distance to default} $\downarrow$ $\Rightarrow$ greater fragility
\end{enumerate}
