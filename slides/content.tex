...
\begin{frame}{Abstract: ABM, Interbank Market, and Contagion}
  \begin{itemize}
    \item \textbf{Agent-Based Model} to study systemic risk in interbank credit networks, with two key parameters:
    \begin{itemize}
    \item \textbf{Probability of attachment} $p$ of Erd\H{o}s--R\'enyi graphs as interbank connections generator (exogenous).
    \item \textbf{Interest rate} $r$ (endogenous).    
    \end{itemize}
    \item As $p$ increases, the network becomes more connected and $r$ declines due to reduced market power $\psi$.
    \item However, $p$ raises the probability of borrower bankruptcy $p_b$: excessively low interest rates, maintained for too long \textbf{reduces the stability of the system}.
  \end{itemize}
\end{frame}
...
\begin{frame}{The Risk-Sharing Trade-off}
  ...
  \begin{itemize}
    ...
    \item \textbf{Cost:} Excessive connectivity increases \textbf{systemic risk}---default propagates through the network, and lenders serving a larger number of clients face higher bankruptcies.
  \end{itemize}
  A \textbf{non-monotonic relationship} exists---low connectivity stabilizes by diluting shocks, but high connectivity facilitates cascading failures. Taken together, these effects imply that excessively low interest rates increase both bankruptcies and contagion.
\end{frame}
...
\begin{frame}{Failure Channels in Financial Crises}
  In literature, three primary propagation mechanisms of systemic failure:
  \begin{enumerate}
...
%   The three failure channels of the model:
%   \begin{itemize}
...
{\usebackgroundtemplate{%
  \vbox to 0pt{\vskip40pt\hbox{\hskip30pt\includegraphics[width=0.9\paperwidth,height=0.85\paperheight,keepaspectratio]{with_directions.png}}\vss}%
}
\begin{frame}[t]
  ...
  \begin{minipage}{0.85\textwidth}
    \vspace{3.5cm}
    \footnotesize
    Each of the $\binom{N}{2}$ edges is included independently with probability $p$. For $N=20$ banks:
    \begin{itemize}
      \item $p = 0$: isolated banks, no interbank market.
      \item $p = 1/N = 0.05$: \textbf{phase transition}, when giant component begins to emerge.
...
\begin{frame}{Network structure}
  \begin{columns}[T]
    \begin{column}{0.40\textwidth}
      \centering
      Erd\H{o}s--R\'enyi generated for $N=20$ \\
      \includegraphics[width=\textwidth]{s01_erdos_renyi_10b.png}
    \end{column}
    \begin{column}{0.53\textwidth}
      \centering
      Considered links after applying restrictions\\
      \includegraphics[width=\textwidth, trim=0 0 0 40, clip]{s01_erdos_renyi_10.png}
    \end{column}
  \end{columns}
...
\begin{frame}{Equity Evolution}
  ...
  \begin{itemize}
    ...
    \item \textbf{Fire sales:} last-resort option and only of $E$.
    \item Failed banks replaced by new entrants with small initial capital (initial conditions).
...
\begin{frame}{Failure Channels}
  Three distinct causes of bank failure accounted in the model:
  \begin{enumerate}
...
\begin{frame}{Phase Transition}
  \begin{itemize}
    \item \textbf{Phase transition:} GCS jumps from $\sim 1$ to $\sim 46$ banks ($92\%$) as $p$ crosses the phase transition.
...
\begin{frame}[shrink=0.1]{Phase Transition}
  ...
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      \begin{tikzpicture}
        ...
        \draw[red, dotted, thick] (axis cs:0.055,0) -- (axis cs:0.055,55)
          node[pos=0.5, right, font=\tiny]{$p_t$};
        \end{axis}
      \end{tikzpicture}
      \vspace{0.1cm}      
      $N=50$ banks, {        
      ...
      \vspace{0.2cm} \\
      \footnotesize          
      \textbf{Percolation threshold} $\langle k \rangle \approx 1$ for Erd\H{o}s--R\'enyi graph: \[
        p_c = \frac{1}{N} = \frac{1}{50} = 0.02
        \] \\
      In pruned graph, \textbf{phase transition} observed at $p = 0.055$, $\langle k \rangle = 2.14$ \\
    ...
    \begin{column}{0.42\textwidth}
      ...
      \begin{tabular}{rrr}
        \toprule
        $p$ & \scriptsize orig GCS & \scriptsize pruned\\
        \midrule
        0.00001 & 1 & 0 \\
        0.011 & 5 & 1 \\
        \textbf{0.022} & \textbf{15} & 4 \\
        0.033 & 31 & 8 \\
        0.044 & 41 & 12 \\
        \textbf{0.055} & 46 & \textbf{16} \\
        0.065 & 48 & 25 \\
        0.087 & 49 & 33 \\
        0.110 & 50 & 39 \\
        $\geq$ 0.15 & 50 & 50 \\
...
\begin{frame}{Bankruptcies}
  ...
  \renewcommand{\arraystretch}{0.85}
  Average number of failures during simulations. \\ Initial conditions for each bank:
  \begin{tabular}{l|l}
    \multicolumn{1}{c|}{\textbf{Assets}} & \multicolumn{1}{c}{\textbf{Liabilities}} \\ \midrule
    $C^i_0= 5-0.18 = 4.82$ & $D^i_0= 9$\\
    $L^i_0= 5$ & $E^i_0= 1$ \\
...
\begin{frame}{Market power $\psi$}
  \tiny Market power $\psi$ decreases monotonically as connectivity $p$ increases. 
  In a denser network, the monopolistic markup is eroded. This is the mechanism through which connectivity drives interest rates downward.
  \begin{columns}[T]
    \begin{column}{0.75\textwidth}
            \begin{figure}
        \centering
...
\begin{frame}{Probability of bankruptcy $p_b$}
  \tiny As connectivity increases, the probability of borrower bankruptcy rises. 
  Lenders attempt to serve a larger number of clients and at the same time the system is being more fragile.
  \begin{columns}[T]
    \begin{column}{0.75\textwidth}
      \begin{figure}
        \centering
...
\begin{frame}{Bad debt}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.23\textwidth}
      ...
      \end{tabular}
             \\ 
            Average sum of bad debt in the simulations. 
...
\begin{frame}{Findings (1/2)}
  As connectivity $p$ increases:
  \begin{enumerate}
    %\item The Giant Component Size increases sharply above the phase transition.
    \item Market power decreases with $p$, and the equilibrium interest rate falls also.
...
\begin{frame}{From here: reinforce learning for policy implications}
  \begin{itemize}
    \item \textbf{Heterogeneity} is a root cause of systemic instability (failure of large banks).
    \item Interbank lending restricted among banks with \textbf{similar liquidity characteristics}.
    \item Reserve requirements dampen cascades but will \textbf{constrain growth}.
