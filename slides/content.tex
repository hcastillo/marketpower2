...
\begin{frame}{Abstract: ABM, Interbank Market, and Contagion}
  \begin{itemize}
    ...
    \item The dynamics are driven by two key parameters: the \textbf{probability of attachment} $p$ of Erdős–Rényi graphs as interbank connections generator (exogenous) and the \textbf{interest rate} $r$ (endogenous).
...
\begin{frame}{Channels of Contagion in Financial Crises}
  ...
  \end{enumerate}
  \vspace{0.3cm}
  The model distinguishes three failure channels: \textbf{rationing} (borrower cannot secure sufficient funding), \textbf{contagion} (lender suffers losses from borrower default), and \textbf{repayment failure}. At low connectivity, rationing dominates; at high connectivity, repayment failure becomes the primary risk.
\end{frame}
...
\begin{frame}{Model Overview}
  \begin{itemize}
    ...
    \item Goal: analyze how \textbf{connectivity} ($p$) and \textbf{market power} affect system stability through the endogenous adjustment of $r$.
  \end{itemize}
\end{frame}
...
\begin{frame}{Interest Rate Derivation}
  ...
  \begin{itemize}
    ...
    \item As connectivity $p$ increases, competition intensifies, market power $\psi$ declines, and the equilibrium interest rate falls.
  \end{itemize}
\end{frame}
...
\begin{frame}[shrink=0.1]{Phase Transition}
  \begin{itemize}
    \item \textbf{Percolation threshold:} GCS jumps from $\sim 1$ to $\sim 46$ banks ($92\%$) as $p$ crosses the critical value $1/N$.
    \item \textbf{Low $p$:} rationing-dominated, high interest rates ($r \approx 30$), few loans granted.
